...
def train(args):
    ...
    if args.resume:
        ckpt = os.path.join(args.resume_ckpt, str(args.resume) + '.pth')
    ...
    print('Train length: {}, Validation length: {}'.format(train_len, val_len))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with tqdm(total=len(trainset), desc='Epoch {}'.format(epoch+1), ncols=60) as pbar:
            for i, batch in enumerate(trainset):
                clear, hazy, trans, A= [x.cuda().float() for x in batch]
                optimizer.zero_grad()
                dehaze_est, trans_est = model(hazy)
                ...
                if i % 2000 == 0: 
                    torch.save({'model_state_dict': model.state_dict(),
                                ...
                                }, os.path.join(args.ckpt, str(epoch+1)+'_'+str(i//2000)+".pth" ))                  
                pbar.update(1)
        model.eval()
        PSNR, SSIM = [], []
        ...
        torch.save({'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'lr_scheduler_state_dict': scheduler.state_dict()
                    }, os.path.join(args.ckpt, str(epoch+1)+'.pth'))
...
def get_args():
    ...
    parser.add_argument('--ckpt', type=str, default='/data/dehaze/ckpt/outdoor-vlb')
    parser.add_argument('--resume_ckpt', type=str, default='/data/dehaze/ckpt/outdoor-vlb')
    parser.add_argument('--epoch', type=int, default=30)
    parser.add_argument('--log_dir', type=str, default='/data/dehaze/log/outdoor-vlb3')
    ...
    parser.add_argument('--resume', type=int, default=11)
