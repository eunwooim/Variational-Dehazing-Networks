...
class DehazeFormer(nn.Module):
	def __init__(self, in_chans=4, out_chans=4, window_size=8,
		...
		self.patch_unembed = PatchUnEmbed(
			patch_size=1, out_chans=last_chans, embed_dim=embed_dims[4], kernel_size=3)
...
def dehazeformer_b(last_chans=4):
    return DehazeFormer(
        embed_dims=[24, 48, 96, 48, 24],
		mlp_ratios=[2., 4., 4., 2., 2.],
		depths=[16, 16, 16, 8, 8],
...
class VDD(nn.Module): 
	def __init__(self): 
		...
		self.DNet = dehazeformer_b()
		self.TNet = dehazeformer_b(last_chans=1)
...
# 	input_tensor = torch.rand(16,3,460,620)
# 	network = VDD()
# 	dehaze, trans = network(input_tensor)
# 	print(dehaze.shape, trans.shape)