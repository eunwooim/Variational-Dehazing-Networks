...
class FFANet(nn.Module):
    def __init__(self, gps=3, blocks=19, conv=default_conv, input_dim = 3):
        super(FFANet, self).__init__()
        self.gps = gps
        self.dim = 64
        kernel_size = 3
        pre_process = [conv(input_dim, self.dim, kernel_size)]
        assert self.gps == 3
...
class VHRN(nn.Module):
    ...
    def forward(self, x, mode='train'):
        ...
        if mode.lower() == 'transmission':
            phi_T= self.sigmoid(self.TNet(x))
            return phi_T

class VGF(nn.Module):
    def __init__(self): 
        super(VGF, self).__init__()
        self.DNet = GCANet(4, 1, mode='d')
        self.TNet = FFANet(input_dim =4)
